...
class CandlePlot:
    def __init__(self, df, candles=True):
        ...
        self.create_candle_fig()  # Call method correctly
        self.show_plot()  

    def add_timestr(self):
        self.df_plot['sTime'] = [dt.datetime.strftime(x, "%y-%m-%d %H:%M") 
                        for x in self.df_plot.time]  # Fix incorrect df reference

    def create_candle_fig(self):
        self.add_timestr()
        self.fig = make_subplots(specs=[[{"secondary_y": True}]])  # Initialize fig
    ...
    def show_plot(self, width=900, height=400, nticks=5, line_traces=[], sec_traces=[]):
        if line_traces is None:
            line_traces = []
        self.add_traces(line_traces, is_sec=True)
