...
def keltnerchannels(df: pd.DataFrame, n_ema=20, n_atr=10):
    ...
    df['ATR'] = ATR(df, n=n_atr)['ATR']
    df['KC_UP'] = df['EMA'] + df['ATR']
    df['KC_LW'] = df['EMA'] - df['ATR']
    df.drop(columns=['EMA', 'ATR'], inplace=True)
    return df
